  ...
  Future<void> init() async {
    ...
    if (_defaultSavePath.isEmpty) {
      if (Platform.isAndroid) {
        _defaultSavePath = '/storage/emulated/0/Download/DirXplore Pro';
      } else if (Platform.isIOS) {
        try {
          final path = await _iosChannel.invokeMethod<String>('getSavePath');
          _defaultSavePath = path ?? '${(await getApplicationDocumentsDirectory()).path}/DirXplore Pro';
        } catch (_) {
          final dir = await getApplicationDocumentsDirectory();
          _defaultSavePath = '${dir.path}/DirXplore Pro';
        }
      } else {
        try {
          final dir = await getApplicationDocumentsDirectory();
