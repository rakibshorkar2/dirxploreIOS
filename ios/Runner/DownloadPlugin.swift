...
class DownloadPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let eventChannel = FlutterEventChannel(name: "com.dirxplorerakib.pro/ios_download_events", binaryMessenger: messenger)
        eventChannel.setStreamHandler(DownloadManager.shared)

        let liveActivityChannel = FlutterMethodChannel(name: "com.dirxplorerakib.pro/live_activity", binaryMessenger: messenger)
        registrar.addMethodCallDelegate(instance, channel: liveActivityChannel)

        let liveActivityErrorChannel = FlutterEventChannel(name: "com.dirxplorerakib.pro/live_activity_errors", binaryMessenger: messenger)
        liveActivityErrorChannel.setStreamHandler(instance)

        let proxyChannel = FlutterMethodChannel(name: "com.dirxplorerakib.pro/proxy_config", binaryMessenger: messenger)
        registrar.addMethodCallDelegate(instance, channel: proxyChannel)

        let notificationChannel = FlutterMethodChannel(name: "com.dirxplorerakib.pro/notifications", binaryMessenger: messenger)
        registrar.addMethodCallDelegate(instance, channel: notificationChannel)

        let backgroundChannel = FlutterMethodChannel(name: "com.dirxplorerakib.pro/background_services", binaryMessenger: messenger)
        registrar.addMethodCallDelegate(instance, channel: backgroundChannel)

        TorrentBridgeHandler.register(with: registrar)
        if #available(iOS 16.2, *) {
            TorrentBackgroundManager.register(with: registrar)
        }
    }
    ...
    func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "getSavePath":
            if let persistentURL = DownloadManager.shared.persistentFolderURL {
                ...
                let dir = documentsDir.appendingPathComponent("DirXplore Pro", isDirectory: true)
